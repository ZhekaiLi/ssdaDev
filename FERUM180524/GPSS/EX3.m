...
probdata.marg = [ 1*ones(n,1) zeros(n,1) ones(n,1) ones(n,1) nan*ones(n,4) zeros(n,1) ];
...
% GP surrogate
GPmodel.ktype                = 'poly';   % kernal type: sqexp, ratquad, poly, matern
GPmodel.dim                  = n;        % dimension of RVs
...
gfundata(1).expression = [ '-(x' sprintf('%d+x', 1:(n-1)) sprintf('%d)/%.10e+%.10e',n,sqrt(n),-norminv(1e-3,0,1)) ];
