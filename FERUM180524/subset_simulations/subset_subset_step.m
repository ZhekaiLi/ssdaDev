...
subgermU = data.U(:,Indgerm);  % seed for conditional sampling
subgermG = data.G(Indgerm);  % value of limit state function

subsetU   = [];  % store newly generated U
subsetG  = [];
...
while size(subsetU,2) < num_sim
   ...
   [subtempu, I_eval, newG, ratio ] = subset_GCS(subgermU,width,rand_generator);  % generate one random sample
   MHrate = [ MHrate ; 100*mean(ratio,2) 100*sum(newG,2)/length(newG) ];   % general acceptance ratio in each row
   ...
   subtempu_eval = subtempu(:,I_eval);  % samples need to be evaluated
   subtempg = subgermG;
