...
switch marg(i,1)
   ...
   case 3  % Gamma distribution
      ...
      zi = inv_norm_cdf( ferum_cdf(3,X,lambda,k) );
      dzdlambda = ((X.*(lambda*X).^(k-1))/gamma(k)).*exp(-lambda*X) ./ ferum_pdf(1,zi,0,1);
      % Computed dzdk with a forward finite difference scheme
      h = 1/10000;
      cc = ( ferum_cdf(3,X,lambda,k+h) - ferum_cdf(3,X,lambda,k) ) / h;
   ...
   case 4  % Shifted exponential distribution
      ...
      zi = inv_norm_cdf( ferum_cdf(4,X,lambda,x_zero) );
      dzdlambda = (X-x_zero) .* exp(-lambda*(X-x_zero)) ./ ferum_pdf(1,zi,0,1);
      dzdx_zero = -lambda * exp(-lambda*(X-x_zero)) ./ ferum_pdf(1,zi,0,1);
   ...
   case 5  % Shifted Rayleigh distribution
      ...
      zi = inv_norm_cdf( ferum_cdf(5,X,a,x_zero) );
      dzda = -((X-x_zero).^2/a^3).*exp(-0.5*((X-x_zero)/a).^2) ./ ferum_pdf(1,zi,0,1);
      dzdx_zero = -((X-x_zero)/a^2).*exp(-0.5*((X-x_zero)/a).^2) ./ ferum_pdf(1,zi,0,1);
   ...
   case 6  % Uniform distribution
      ...
      zi = inv_norm_cdf( ferum_cdf(6,X,a,b) );
      dzda = ((X-b)/(b-a)^2) ./ ferum_pdf(1,zi,0,1);
      dzdb = -((X-a)/(b-a)^2) ./ ferum_pdf(1,zi,0,1);
   ...
   case 8 % Chi-square distribution
      ...
      zi = inv_norm_cdf( ferum_cdf(8,X,nu) );
      dzdlambda = 0;
      % Computed dzdnu with a forward finite difference scheme
      h = 1/10000;
      cc = ( ferum_cdf(8,X,nu+h) - ferum_cdf(8,X,nu) ) / h;
   ...
   case 11 % Type I largest value distribution ( same as Gumbel distribution )
      ...
      zi = inv_norm_cdf( ferum_cdf(11,X,u_n,a_n) );
      dzdu_n = -a_n * exp(-a_n*(X-u_n)) .* exp(-exp(-a_n*(X-u_n))) ./ ferum_pdf(1,zi,0,1);
      dzda_n = (X-u_n) .* exp(-a_n*(X-u_n)) .* exp(-exp(-a_n*(X-u_n))) ./ ferum_pdf(1,zi,0,1);
   ...
   case 12 % Type I smallest value distribution
      ...
      zi = inv_norm_cdf( ferum_cdf(12,X,u_1,a_1) );
      dzdu_1 = -a_1 * exp(a_1*(X-u_1)) .* exp(-exp(a_1*(X-u_1))) ./ ferum_pdf(1,zi,0,1);
      dzda_1 = (X-u_1) .* exp(a_1*(X-u_1)) .* exp(-exp(a_1*(X-u_1))) ./ ferum_pdf(1,zi,0,1);
   ...
   case 13 % Type II largest value distribution
      ...
      zi = inv_norm_cdf( ferum_cdf(13,X,u_n,k) );
      dzdu_n = (-(u_n./X).^k) * (k/u_n) .* exp(-(u_n./X).^k) ./ ferum_pdf(1,zi,0,1);
      dzdk = (-(u_n./X).^k) .* log(u_n./X) .* exp(-(u_n./X).^k) ./ ferum_pdf(1,zi,0,1);
   ...
   case 14 % Type III smallest value distribution
      ...
      zi = inv_norm_cdf( ferum_cdf(14,X,u_1,k,epsilon) );
      cst1 = ((X - epsilon)/(u_1 - epsilon)).^k ;
      cst2 = exp(-cst1);
      cst3 = (X - epsilon)/(u_1 - epsilon)^2 - 1/(u_1 - epsilon);

      dzdu_1 = -cst1 .* cst2 * (k/(u_1-epsilon)) ./ ferum_pdf(1,zi,0,1);
      dzdk = cst1 .* cst2 .* log((X - epsilon)/(u_1 - epsilon)) ./ ferum_pdf(1,zi,0,1);
      dzdepsilon = cst1 .* cst2 * k .* cst3 * (u_1 - epsilon) ./ ( (X-epsilon) .* ferum_pdf(1,zi,0,1) );
   ...
   case 15 % Gumbel distribution ( same as type I largest value distribution )
      ...
      zi = inv_norm_cdf( ferum_cdf(15,X,u_n,a_n) );
      dzdu_n = -a_n * exp(-a_n*(X-u_n)) .* exp(-exp(-a_n*(X-u_n))) ./ ferum_pdf(1,zi,0,1);
      dzda_n = (X-u_n) .* exp(-a_n*(X-u_n)) .* exp(-exp(-a_n*(X-u_n))) ./ ferum_pdf(1,zi,0,1);
   ...
   case 16 % Weibull distribution ( same as Type III smallest value distribution with epsilon = 0 )
      ...
      zi = inv_norm_cdf( ferum_cdf(16,X,u_1,k) );
      dzdu_1 = -((X/u_1).^k) * (k/u_1) .* exp(-(X/u_1).^k) ./ ferum_pdf(1,zi,0,1);
      dzdk = ((X/u_1).^k) .* log(X/u_1) .* exp(-(X/u_1).^k) ./ ferum_pdf(1,zi,0,1);
