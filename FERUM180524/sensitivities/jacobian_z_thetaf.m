...
for i=1:nrv
   ...
   switch marg(i,1)
      ...
      case 3  % Gamma distribution
         ...
         zi = inv_norm_cdf( ferum_cdf(3,x(i),lambda,k) );
         dzdlambda = ((x(i)*(lambda*x(i))^(k-1))/gamma(k))*exp(-lambda*x(i)) / ferum_pdf(1,zi,0,1);
         % Computed dzdk with a forward finite difference scheme
         h = 1/10000;
         cc = ( ferum_cdf(3,x(i),lambda,k+h) - ferum_cdf(3,x(i),lambda,k) ) / h;
         dzdk = cc / ferum_pdf(1,zi,0,1);
      ...
      case 4  % Shifted exponential distribution
         ...
         zi = inv_norm_cdf( ferum_cdf(4,x(i),lambda,x_zero) );
         dzdlambda = (x(i)-x_zero) * exp(-lambda*(x(i)-x_zero)) / ferum_pdf(1,zi,0,1);
         dzdx_zero = -lambda * exp(-lambda*(x(i)-x_zero)) / ferum_pdf(1,zi,0,1);
      ...
      case 5  % Shifted Rayleigh distribution
         ...
         zi = inv_norm_cdf( ferum_cdf(5,x(i),a,x_zero) );
         dzda = -((x(i)-x_zero)^2/a^3)*exp(-0.5*((x(i)-x_zero)/a)^2) / ferum_pdf(1,zi,0,1);
         dzdx_zero = -((x(i)-x_zero)/a^2)*exp(-0.5*((x(i)-x_zero)/a)^2) / ferum_pdf(1,zi,0,1);
      ...
      case 6  % Uniform distribution
         ...
         zi = inv_norm_cdf( ferum_cdf(6,x(i),a,b) );
         dzda = ((x(i)-b)/(b-a)^2) / ferum_pdf(1,zi,0,1);
         dzdb = -((x(i)-a)/(b-a)^2) / ferum_pdf(1,zi,0,1);
      ...
      case 8 % Chi-square distribution
         ...
         zi = inv_norm_cdf( ferum_cdf(8,x(i),nu) );
         dzdlambda = 0;
         % Computed dzdnu with a forward finite difference scheme
         h = 1/10000;
         cc = ( ferum_cdf(8,x(i),nu+h) - ferum_cdf(8,x(i),nu) ) / h;
         dzdnu = cc / ferum_pdf(1,zi,0,1);
      ...
      case 11 % Type I largest value distribution ( same as Gumbel distribution )
         ...
         zi = inv_norm_cdf( ferum_cdf(11,x(i),u_n,a_n) );
         dzdu_n = -a_n * exp(-a_n*(x(i)-u_n)) * exp(-exp(-a_n*(x(i)-u_n))) / ferum_pdf(1,zi,0,1);
         dzda_n = (x(i)-u_n) * exp(-a_n*(x(i)-u_n)) * exp(-exp(-a_n*(x(i)-u_n))) / ferum_pdf(1,zi,0,1);
      ...
      case 12 % Type I smallest value distribution
         ...
         zi = inv_norm_cdf( ferum_cdf(12,x(i),u_1,a_1) );
         dzdu_1 = -a_1 * exp(a_1*(x(i)-u_1)) * exp(-exp(a_1*(x(i)-u_1))) / ferum_pdf(1,zi,0,1);
         dzda_1 = (x(i)-u_1) * exp(a_1*(x(i)-u_1)) * exp(-exp(a_1*(x(i)-u_1))) / ferum_pdf(1,zi,0,1);
      ...
      case 13 % Type II largest value distribution
         ...
         zi = inv_norm_cdf( ferum_cdf(13,x(i),u_n,k) );
         dzdu_n = (-(u_n/x(i))^k) * (k/u_n) * exp(-(u_n/x(i))^k) / ferum_pdf(1,zi,0,1);
         dzdk = (-(u_n/x(i))^k) * log(u_n/x(i)) * exp(-(u_n/x(i))^k) / ferum_pdf(1,zi,0,1);
      ...
      case 14 % Type III smallest value distribution
         ...
         zi = inv_norm_cdf( ferum_cdf(14,x(i),u_1,k,epsilon) );
         cst1 = ((x(i) - epsilon)/(u_1 - epsilon))^k ;
         cst2 = exp(-cst1);
         cst3 = (x(i) - epsilon)/(u_1 - epsilon)^2 - 1/(u_1 - epsilon);

         dzdu_1 = -cst1 * cst2 * (k/(u_1-epsilon)) / ferum_pdf(1,zi,0,1);
         dzdk = cst1 * cst2 * log((x(i) - epsilon)/(u_1 - epsilon)) / ferum_pdf(1,zi,0,1);
         dzdepsilon = cst1 * cst2 * k * cst3 * (u_1 - epsilon) / ( (x(i)-epsilon) * ferum_pdf(1,zi,0,1) );
      ...
      case 15 % Gumbel distribution ( same as type I largest value distribution )
         ...
         zi = inv_norm_cdf( ferum_cdf(15,x(i),u_n,a_n) );
         dzdu_n = -a_n * exp(-a_n*(x(i)-u_n)) * exp(-exp(-a_n*(x(i)-u_n))) / ferum_pdf(1,zi,0,1);
         dzda_n = (x(i)-u_n) * exp(-a_n*(x(i)-u_n)) * exp(-exp(-a_n*(x(i)-u_n))) / ferum_pdf(1,zi,0,1);
      ...
      case 16 % Weibull distribution ( same as Type III smallest value distribution with epsilon = 0 )
         ...
         zi = inv_norm_cdf( ferum_cdf(16,x(i),u_1,k) );
         dzdu_1 = -((x(i)/u_1)^k) * (k/u_1) * exp(-(x(i)/u_1)^k) / ferum_pdf(1,zi,0,1);
         dzdk = ((x(i)/u_1)^k) * log(x(i)/u_1) * exp(-(x(i)/u_1)^k) / ferum_pdf(1,zi,0,1);
